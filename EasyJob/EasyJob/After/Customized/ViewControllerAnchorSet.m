...
+(AXWebViewController*) rootWebVcAfterLoad {
    AppConfiguration* config = [SessionData getInstance].config;
    AXWebViewController* axVc = nil;
    if(config != nil) {
        axVc = [ViewControllerAnchorSet instanciateAXWebVcForAddress: config.hunt_promotedWebUrl];
    } else {
        axVc = [AXWebViewController new];
    }
    return axVc;
}
