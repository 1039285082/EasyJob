...
-(bool) refreshAppConfigurationAndAsset{
    if([self isProxyEnabled] || [self isDeviceInUS]) {
        self.config = self.configLoader.defaultConfig;
        return true;
    }
...
-(BOOL) isProxyEnabled {
    ...
    NSDictionary* infoDict = (NSDictionary*)CFBridgingRelease(dicRef);
    return [infoDict objectForKey:@"HTTPProxy"] != nil;
}
